...
import (
	...
	cutil "github.com/DgamesFoundation/subchain/fabric/core/container/util"
...
func writeChaincodePackage(spec *pb.ChaincodeSpec, tw *tar.Writer) error {
	...
	newRunLine := fmt.Sprintf("RUN go install %s && cp src/github.com/DgamesFoundation/subchain/fabric/peer/core.yaml $GOPATH/bin && mv $GOPATH/bin/%s $GOPATH/bin/%s", urlLocation, chaincodeGoName, spec.ChaincodeID.Name)
